...
import { vendorHeaders } from './vendorHeader';
...
export class VendorGridComponent implements OnInit{
  ...
  addProvider:string=`/${appRoutes.vendor.base}/${appRoutes.vendor.add}`
  // addProvider:string=`vendor/add`
  ...
  dataSource: IDatasource = {
    getRows: (params: IGetRowsParams) => {
      ...
      this.fetchDataAndLoadProvider(params);
    },
  };
  ...
  fetchDataAndLoadProvider(params: IGetRowsParams, searchObj?: any) {
    const filtersObj = { ...this.uiState.filters };
    let sub = this.providerService.getAllProviders(filtersObj).subscribe((res) => {
  ...
  updateDataSource(searchObj?: any) {
    this.dataSource = {
      getRows: (params: IGetRowsParams) => {
        this.gridApi.showLoadingOverlay();
        this.fetchDataAndLoadProvider(params, searchObj);
      },
    };
    this.gridApi.setGridOption('datasource', this.dataSource);
  }
